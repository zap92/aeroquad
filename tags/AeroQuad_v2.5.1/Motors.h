...
public:
  ...
  void initialize(void)
  {
    ...
    commandAllMotors(1000);                                     // Initialise motors to 1000us (stopped)
      ...
      #endif
      #if (LASTMOTOR == 8)                                      // JI - 11/25/11
        // Init PWM Timer 1  16 bit                             // JI - 11/25/11
        TCCR1A = (1<<WGM11)|(1<<COM1A1)|(1<<COM1B1);            // JI - 11/25/11
        TCCR1B = (1<<WGM13)|(1<<WGM12)|(1<<CS11);               // JI - 11/25/11
        ICR1 = PWM_COUNTER_PERIOD;                              // JI - 11/25/11
      #endif                                                    // JI - 11/25/11
   #else
