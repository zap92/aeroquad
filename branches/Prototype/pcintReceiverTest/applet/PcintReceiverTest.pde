...
int receiverChannel[6] = {ROLLPIN, THROTTLEPIN, PITCHPIN, YAWPIN, MODEPIN, AUXPIN};
...
void loop() {
  ...
  if (currentTime > (previousTime + 100)) {
    for (channel = ROLL; channel < AUX; channel++) {
      Serial.print(readReceiver(receiverChannel[channel]));
      Serial.print(", ");
    }
    Serial.println(readReceiver(receiverChannel[AUX]));
  }
}
