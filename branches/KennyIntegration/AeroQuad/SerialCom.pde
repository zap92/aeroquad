  ...
  if (SERIAL_AVAILABLE()) {
    ...
    case '5': // Send individual motor commands (motor, command)
      if (validateCalibrateCommand())
      {
        ...
        for (byte motor = FIRSTMOTOR; motor < LASTMOTOR; motor++)
          motorCommand[motor] = constrain(remoteMotorCommand[motor], 1000, 2000);
...
void sendSerialTelemetry() {
  ...
  case 'o': // Send accelerometer cal values
    SERIAL_PRINT(readFloat(XAXIS_ACCEL_SCALE_FACTOR_ADR), 6);
    comma();
    SERIAL_PRINT(readFloat(XAXIS_ACCEL_BIAS_ADR), 6);
    comma();
    SERIAL_PRINT(readFloat(YAXIS_ACCEL_SCALE_FACTOR_ADR), 6);
    comma();
    SERIAL_PRINT(readFloat(YAXIS_ACCEL_BIAS_ADR), 6);
    comma();
    SERIAL_PRINT(readFloat(ZAXIS_ACCEL_SCALE_FACTOR_ADR), 6);
    comma();
    SERIAL_PRINTLN(readFloat(ZAXIS_ACCEL_BIAS_ADR), 6);
    queryType = 'X';
    break;
