...
Accelerometer_IDG500::Accelerometer_IDG500() {
  accelScaleFactor = G_2_MPS2((3.0/1024.0) / 0.300);  // force aref to 3.0 for v1.7 shield
  smoothFactor     = 1.0;
}

void Accelerometer_IDG500::setAref(float aref) {
	this->aref = aref;
	accelScaleFactor = G_2_MPS2((aref/1024.0) / 0.300);
}
  
void Accelerometer_IDG500::measure(void) {
  // rollChannel = 1
  // pitchChannel = 0
  // zAxisChannel = 2
...
void Accelerometer_IDG500::calibrate() {
  // rollChannel = 1
  // pitchChannel = 0
  // zAxisChannel = 2
