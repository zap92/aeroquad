...
SoftwareServo::SoftwareServo() : pin(0),angle(NO_ANGLE),pulse0(0),min16(62),max16(125),next(0)
{}
...
{
	// If angleArg>=180 interpret it as a uS interval and convert to degrees
	if (angleArg>180) {
		angleArg = (angleArg - 1000)*0.18;
	}
...
void SoftwareServo::refresh()
{
    ...
    // if we haven't wrapped millis, and 20ms (PRJ: Changed to 10ms) have not passed, then don't do anything
    if ( m >= lastRefresh && m < lastRefresh + 10) return;
    lastRefresh = m;
